...
import { CheckCircle2, ArrowRight, Wrench, Building2, ChevronRight } from "lucide-react";
    ...
    <V2InnerShell>
      ...
      <div className="grid gap-8 lg:grid-cols-12 lg:items-center">
        <div className="lg:col-span-7">
          ...
          <p className="mt-4 text-base leading-7 text-[var(--muted2)]">{service.summary}</p>
      ...
      <div className="mt-10 grid grid-cols-3 gap-3">
        {service.stats.map((s) => (
          ...
          >
            <div className="text-2xl font-semibold tracking-tight text-[var(--accent)]">{s.value}</div>
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg2)] p-6 shadow-[0_8px_40px_rgba(0,0,0,0.2)] lg:col-span-2">
          ...
          <ul className="mt-4 grid gap-2 sm:grid-cols-2">
            {service.bullets.map((b) => (
              <li key={b} className="flex items-start gap-2.5 text-sm leading-6 text-[var(--muted2)]">
          ...
          <ul className="mt-4 grid gap-4">
            {service.outcomes.map((o, i) => (
              <li key={o} className="flex items-start gap-3">
                ...
                <span className="text-sm leading-6 text-[var(--muted2)]">{o}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      {/* ── Our Process ── */}
      <div className="mt-12">
        <div className="text-xs font-semibold uppercase tracking-wider text-[var(--muted)]">How we work</div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {service.process.map((step) => (
            ...
            >
              ...
              <p className="mt-2 text-xs leading-5 text-[var(--muted2)]">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-8">
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-semibold tracking-tight text-[var(--text)]">Industries served</div>
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
      ...
      {relatedServices.length > 0 && (
        <div className="mt-12">
          <div className="text-sm font-semibold tracking-tight text-[var(--text)]">Related services</div>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-1 text-xs italic text-[var(--muted)]">{svc.tagline}</div>
                  </div>
                </Link>
              );
            })}
          ...
          <div className="grid gap-6 lg:grid-cols-12 lg:items-center">
            <div className="lg:col-span-8">
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--muted2)]">
                Tell us about your engineering challenge and we&apos;ll map the right delivery model,
                tooling, and team structure for your context.
              </p>
            </div>
      ...
      <div className="mt-6">
        <Link href="/services" className="text-sm text-[var(--muted)] transition hover:text-[var(--text)]">
          ← All services
        </Link>
      </div>
    </V2InnerShell>
