...
export function getClientLogoSources(domain: string): string[] {
  ...
  if (domain === "whalecloud.com") {
    targetDomain = "www.iwhalecloud.com";
  } else if (domain === "harman.com") {
    targetDomain = "harmanaudio.com";
  } else if (domain === "payzapp.in") {
    targetDomain = "hdfcbank.com";
  } else if (domain === "olacabs.com") {
    targetDomain = "olaelectric.com";
  }

  const site = `https://${targetDomain}`;
  const sources: string[] = [];

  const logoDevToken = process.env.NEXT_PUBLIC_LOGO_DEV_TOKEN;
  if (logoDevToken) {
    sources.push(
      `https://img.logo.dev/${encodeURIComponent(targetDomain)}?token=${logoDevToken}&size=160&format=png&retina=true`,
    );
  }
  ...
  sources.push(
    ...
    `https://www.google.com/s2/favicons?domain=${encodeURIComponent(targetDomain)}&sz=128`,
  );
